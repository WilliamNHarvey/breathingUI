define([
  ...
  app.controller('breathsController', function($rootScope, $scope, $location, $window, breathsService, $compile, LS, userService) {
      ...
      var increaseInterval = setInterval(setIncrease, 2500);
      ...
      var restData = (function() {
          ...
          function produceValue(currentTime) {
              ...
              if($scope.recording) {
                  if(!storedData[index].eeg) {
                      storedData[index].eeg = [];
                  }

                  storedData[index].eeg.push([now, lastProducedValue]);
              }
